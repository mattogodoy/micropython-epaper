...
in Flash is employed a build with this capability must be used. This was implemented on the Pyboard
on 13th April 2016.
...
This release uses an open source Python utility to create binary and python fonts. Creating font
files from ``ttf`` or ``otf`` sources is simplified.

This change has enabled the font files to use horizontal mapping providing faster font rendering.
Consequently binary and Python font file formats have changed and the files should be replaced.
This also involves minor changes to applications. See the example code below.
